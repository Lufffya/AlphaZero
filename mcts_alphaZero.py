# -*- coding: utf-8 -*-
"""
Monte Carlo Tree Search in AlphaGo Zero style
which uses a policy-value network to guide the tree search and evaluate the leaf nodes
"""
import numpy as np
import copy
...
class TreeNode:
    ...
    def __init__(self, parent, prior_p):
        ...
        self._n_visits = 0  # a number of node visits
        self._Q = 0  # node action benefit value
        self._u = 0  # UCT algorithm score
        self._P = prior_p  # node Prior probability

    def select(self, c_puct):
        """Select action among children that gives maximum action value Q plus bonus u(P).add()
    ...
    def get_value(self, c_puct):
        ...
        """
        # formula：(Upper Confidence Bound Apply to Tree)
...
class MCTS:
    ...
    def __init__(self, policy_value_fn, c_puct=5, n_playout=1600):
        """
        policy_value_fn: a function that takes in a board state and outputs a list of (action, probability) tuples 
            and also a score in [-1, 1] (i.e. the expected value of the end game score from the current player's perspective) for the current player.
    ...
    def get_move_probs(self, state, temp=1e-3):
        ...
        # calculation the move probabilities based on visit counts at the root node
...
class MCTSPlayer:
    ...
    def __init__(self, policy_value_function, c_puct=5, n_playout=1600, is_selfplay=False):
